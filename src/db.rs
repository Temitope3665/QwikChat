...
pub fn find_user_by_phone(conn: &mut SqliteConnection, user_phone: String) -> Result<Option<User>, DbError> {
...
pub fn get_conversation_by_room_uid(conn: &mut SqliteConnection, uid: Uuid) -> Result<Vec<Conversation>, DbError> {
...
pub fn insert_new_conversation(conn: &mut SqliteConnection, new: NewConversation) -> Result<Conversation, DbError> {
    use crate::schema::conversations::dsl::*;
    use crate::schema::rooms::dsl::{rooms, last_message, id as room_id_col};
    
    let new_conversation = Conversation {
        id: Uuid::new_v4().to_string(),
        user_id: new.user_id,
        room_id: new.room_id.clone(),
        content: new.message.clone(),
        created_at: iso_date(),
    };
    ...
    diesel::update(rooms.filter(room_id_col.eq(new.room_id)))
...
pub fn create_room(conn: &mut SqliteConnection, room_name: &str, participants: Vec<String>) -> Result<Room, DbError> {
