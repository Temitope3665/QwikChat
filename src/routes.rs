...
#[post("/rooms/create")]
pub async fn create_room(pool: web::Data<DbPool>, form: web::Json<models::NewRoom>) -> Result<HttpResponse, Error> {
    let room = web::block(move || {
        let mut conn = pool.get()?;
        db::create_room(&mut conn, &form.name, form.participant_ids.clone())
    })
    .await?
    .map_err(actix_web::error::ErrorUnprocessableEntity)?;
...
#[get("/users/{user_id}")]
pub async fn get_user_by_id(pool: web::Data<DbPool>, id: web::Path<Uuid>) -> Result<HttpResponse, Error> {
...
pub async fn get_conversation_by_id(pool: web::Data<DbPool>, uid: web::Path<Uuid>) -> Result<HttpResponse, Error> {
...
pub async fn get_user_by_phone(pool: web::Data<DbPool>, phone: web::Path<String>) -> Result<HttpResponse, Error> {
